...
def test_rank_decision_signals_uses_kind_and_metric_characteristics() -> None:
    overview = OverviewResponse(
        dataset_id="d1",
        business_model="sales_pipeline",
        business_model_label="Sales pipeline",
        ...
        subheadline="subheadline",
        attention=[
            _insight("money-risk", severity="medium", metric="pipeline_value", value=100000, fields=["amount"]),
            _insight("rate-risk", severity="medium", metric="win_rate", value=42, fields=["stage", "amount"]),
        ],
        opportunities=[
            _insight("money-opportunity", severity="medium", metric="pipeline_value", value=100000, fields=["amount"]),
            _insight("rate-opportunity", severity="medium", metric="win_rate", value=42, fields=["stage", "amount"]),
        ],
    )

    signals = {signal.signal_id: signal for signal in rank_decision_signals(overview)}

    assert signals["money-risk"].impact_score == 100.0
    assert signals["rate-risk"].impact_score == 100.0
    assert signals["money-opportunity"].impact_score == 80.0
    assert signals["rate-opportunity"].impact_score == 73.0
    assert signals["money-risk"].urgency_score == 75.0
    assert signals["money-opportunity"].urgency_score == 65.0
