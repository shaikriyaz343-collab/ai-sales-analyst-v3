...
export async function getSavedIntelligence(datasetId:string,sessionId:string):Promise<import("./types").SavedIntelligenceResponse>{const r=await fetch(`${API_BASE}/api/v1/datasets/${datasetId}/saved?session_id=${encodeURIComponent(sessionId)}`,{cache:"no-store"});if(!r.ok){const b=await r.json().catch(()=>({detail:"Saved intelligence could not be loaded."}));throw new Error(b.detail||"Saved intelligence could not be loaded.");}return r.json();}
export async function saveIntelligence(datasetId:string,sessionId:string,request:{name:string;source_workspace:string;source_id:string;title?:string;summary?:string;metric?:string;dimension?:string}):Promise<import("./types").SavedIntelligence>{const r=await fetch(`${API_BASE}/api/v1/datasets/${datasetId}/saved?session_id=${encodeURIComponent(sessionId)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(request),cache:"no-store"});if(!r.ok){const b=await r.json().catch(()=>({detail:"Intelligence could not be saved."}));throw new Error(b.detail||"Intelligence could not be saved.");}return r.json();}
export async function deleteSavedIntelligence(datasetId:string,sessionId:string,itemId:string){const r=await fetch(`${API_BASE}/api/v1/datasets/${datasetId}/saved/${itemId}?session_id=${encodeURIComponent(sessionId)}`,{method:"DELETE",cache:"no-store"});if(!r.ok){const b=await r.json().catch(()=>({detail:"Saved intelligence could not be deleted."}));throw new Error(b.detail||"Saved intelligence could not be deleted.");}}
